...
from run_feedforward_decode_pid_evaluation import PID_CONFIG
...
PREFERRED_STRATEGY_ORDER = [
    "ff_v2_recommended",
    "ff_v2_decode_pid_adaptive",
    "ff_v2_decode_pid_stable",
    "ff_v2_decode_pid",
    "baseline_350w",
]
...
def aggregate_for_plot(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for source, target in [
        ...
        ("pid_decode_feedback_tbt_ms", "avg_pid_decode_feedback_tbt_ms"),
    ...
    if "avg_pid_decode_feedback_tbt_ms" not in df.columns:
        df["avg_pid_decode_feedback_tbt_ms"] = 0.0
    grouped = (
        ...
        .agg(
            ...
            avg_pid_decode_feedback_tbt_ms=("avg_pid_decode_feedback_tbt_ms", "mean"),
        )
...
def write_report(df: pd.DataFrame, output_path: str):
    ...
    lines = [
        "# Feedforward Decode PID Evaluation Report",
        "",
        "## Summary",
    ...
    pid_names = [name for name in get_plot_strategy_order(df) if name.startswith("ff_v2_decode_pid")]
    if pid_names:
        lines.extend(["", "## Decode PID Behavior"])
    ...
    for pid_name in pid_names:
        ...
        lines.extend([
            ...
            f"mean feedback TBT={pid_df['avg_pid_decode_feedback_tbt_ms'].mean():.2f} ms",
        ])
        ff_df = df[df["strategy"] == "ff_v2_recommended"].set_index(["query_count", "output_length"])
...
def generate_outputs(agg_df: pd.DataFrame, output_dir: str) -> Dict[str, str]:
    ...
    outputs = {
        "ttft": os.path.join(output_dir, "feedforward_decode_pid_ttft.png"),
        "tbt": os.path.join(output_dir, "feedforward_decode_pid_tbt.png"),
        "e2e": os.path.join(output_dir, "feedforward_decode_pid_e2e.png"),
        "energy": os.path.join(output_dir, "feedforward_decode_pid_energy.png"),
        "power": os.path.join(output_dir, "feedforward_decode_pid_power.png"),
        "energy_saving": os.path.join(output_dir, "feedforward_decode_pid_energy_saving.png"),
        "ttft_increase": os.path.join(output_dir, "feedforward_decode_pid_ttft_increase.png"),
        "tbt_increase": os.path.join(output_dir, "feedforward_decode_pid_tbt_increase.png"),
        "e2e_increase": os.path.join(output_dir, "feedforward_decode_pid_e2e_increase.png"),
        "pid_delta": os.path.join(output_dir, "feedforward_decode_pid_delta.png"),
        "pid_feedback_tbt": os.path.join(output_dir, "feedforward_decode_pid_feedback_tbt.png"),
        "pid_updates": os.path.join(output_dir, "feedforward_decode_pid_updates.png"),
        "power_changes": os.path.join(output_dir, "feedforward_decode_pid_power_changes.png"),
        "report": os.path.join(output_dir, "feedforward_decode_pid_report.md"),
    }
    ...
    plot_metric(plot_df, "avg_pid_decode_feedback_tbt_ms", "Mean Decode PID Feedback TBT", "Feedback TBT (ms)", outputs["pid_feedback_tbt"])
...
def parse_args():
    parser = argparse.ArgumentParser(description="Analyze feedforward decode PID evaluation results.")
